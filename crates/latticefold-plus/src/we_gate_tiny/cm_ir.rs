...
// -----------------------------------------------------------------------------
// Disjoint-fragment merge helpers (for parallel shard build, single lowering)
// -----------------------------------------------------------------------------
//
// Many hot paths (notably tiny CM `u_shared`) build lots of independent `CmIr` shards in parallel.
// If we lower each shard independently, the file-backed builder becomes the bottleneck because
// appends/var allocation are effectively serialized at the callsite. Instead, we can merge the
// shards into a single disjoint `CmIr` (pure concatenation with index remaps) and lower once.
//
// This is semantics-preserving: it is exactly the same set of constraints and witness values, in
// the same order as lowering the shards sequentially.

#[inline]
fn remap_var(v: VarRef, local_offset_minus_1: usize) -> VarRef {
    match v {
        VarRef::Base(i) => VarRef::Base(i),
        VarRef::Local(j) => {
            debug_assert!(j > 0);
            VarRef::Local(j + local_offset_minus_1)
        }
    }
}

/// Merge disjoint `CmIr` shards and remap a per-shard **GoldilocksScalar digit** output (17 digits),
/// preserving shard order.
pub(crate) fn merge_disjoint_ir_with_scalar_outputs(
    frags: Vec<(CmIr, ([VarRef; 17], usize))>,
) -> Result<(CmIr, Vec<([VarRef; 17], usize)>), String> {
    let mut merged = CmIr::new();
    let mut outs: Vec<([VarRef; 17], usize)> = Vec::with_capacity(frags.len());

    for (mut ir, (out17, tag)) in frags {
        // Compute local offset into merged.local_asg.
        // `VarRef::Local(1)` corresponds to `local_asg[1]`. We skip each shard's `local_asg[0]` (=ONE),
        // so the first appended local lands at index `merged.local_asg.len()`.
        let local_offset_minus_1 = merged.local_asg.len().saturating_sub(1);

        // Append locals (skip local_asg[0]=ONE).
        if ir.local_asg.len() > 1 {
            merged.local_asg.extend_from_slice(&ir.local_asg[1..]);
        }

        // Remap and append A terms (no reserved slot).
        let a_off = merged.a_terms.len();
        for (coef, v) in ir.a_terms.drain(..) {
            merged.a_terms.push((coef, remap_var(v, local_offset_minus_1)));
        }

        // Remap and append B/C terms (skip reserved 0th slot which is (ONE, Base(0)) / (ZERO, Base(0))).
        let b_off = merged.b_terms.len();
        if ir.b_terms.len() >= 1 {
            for (coef, v) in ir.b_terms.drain(1..) {
                merged.b_terms.push((coef, remap_var(v, local_offset_minus_1)));
            }
        }
        let c_off = merged.c_terms.len();
        if ir.c_terms.len() >= 1 {
            for (coef, v) in ir.c_terms.drain(1..) {
                merged.c_terms.push((coef, remap_var(v, local_offset_minus_1)));
            }
        }

        // Remap constraints' term ranges into merged pools.
        // A ranges always shift by `a_off`.
        // B/C ranges: `0..1` refers to the reserved slot; keep it unchanged. Otherwise, shift by `b_off-1` / `c_off-1`
        // because we did not copy the per-shard reserved element.
        for mut ic in ir.constraints.drain(..) {
            ic.a = (ic.a.start + a_off)..(ic.a.end + a_off);
            if !(ic.b.start == 0 && ic.b.end == 1) {
                let off = b_off.saturating_sub(1);
                ic.b = (ic.b.start + off)..(ic.b.end + off);
            }
            if !(ic.c.start == 0 && ic.c.end == 1) {
                let off = c_off.saturating_sub(1);
                ic.c = (ic.c.start + off)..(ic.c.end + off);
            }
            merged.constraints.push(ic);
        }

        // Merge stats (used for capacity planning / diagnostics).
        merged.stats.linear_constraints += ir.stats.linear_constraints;
        merged.stats.mul_constraints += ir.stats.mul_constraints;
        merged.stats.other_non_linear_constraints += ir.stats.other_non_linear_constraints;
        merged.stats.total_terms_a += ir.stats.total_terms_a;
        merged.stats.total_terms_b += ir.stats.total_terms_b;
        merged.stats.total_terms_c += ir.stats.total_terms_c;
        merged.stats.max_terms_a = merged.stats.max_terms_a.max(ir.stats.max_terms_a);
        merged.stats.max_terms_b = merged.stats.max_terms_b.max(ir.stats.max_terms_b);
        merged.stats.max_terms_c = merged.stats.max_terms_c.max(ir.stats.max_terms_c);

        // Remap output digits.
        let out17_mapped: [VarRef; 17] = core::array::from_fn(|i| remap_var(out17[i], local_offset_minus_1));
        outs.push((out17_mapped, tag));
    }

    Ok((merged, outs))
}

/// Merge disjoint `CmIr` shards and remap a per-shard **RingDigits** output:
/// `[[VarRef; 17]; 64]` (Goldilocks scalar digits per coefficient), preserving shard order.
pub(crate) fn merge_disjoint_ir_with_ring_outputs(
    frags: Vec<(CmIr, [[VarRef; 17]; 64])>,
) -> Result<(CmIr, Vec<[[VarRef; 17]; 64]>), String> {
    let mut merged = CmIr::new();
    let mut outs: Vec<[[VarRef; 17]; 64]> = Vec::with_capacity(frags.len());

    for (mut ir, out_ring) in frags {
        let local_offset_minus_1 = merged.local_asg.len().saturating_sub(1);
        ...
        let out_mapped: [[VarRef; 17]; 64] = core::array::from_fn(|i| {
            core::array::from_fn(|j| remap_var(out_ring[i][j], local_offset_minus_1))
        });
        outs.push(out_mapped);
    }
