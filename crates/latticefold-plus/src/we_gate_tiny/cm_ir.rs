...
impl CmIr {
    ...
    /// Append another IR fragment into `self`, remapping its locals to avoid index collisions.
    ///
    /// Returns the local-index shift applied to `other`'s `VarRef::Local(j)`:
    /// `Local(j)` becomes `Local(j + shift)`.
    ///
    /// This is useful when you build many small IR fragments in parallel but want to **lower once**
    /// into the `Dr1csBuilder` (avoids dozens/hundreds of tiny lowers that serialize range-writes).
    pub(crate) fn append_remap_locals(&mut self, other: CmIr) -> usize {
        // We only support appending IRs of the same "mode". (Mixing would be surprising.)
        debug_assert_eq!(self.count_only, other.count_only);

        let local_shift: usize = self.local_asg.len().saturating_sub(1);

        // Merge stats (exact, used by Pass0 count-only lowering).
        self.stats.linear_constraints = self.stats.linear_constraints.saturating_add(other.stats.linear_constraints);
        self.stats.mul_constraints = self.stats.mul_constraints.saturating_add(other.stats.mul_constraints);
        self.stats.other_non_linear_constraints =
            self.stats.other_non_linear_constraints.saturating_add(other.stats.other_non_linear_constraints);
        self.stats.total_terms_a = self.stats.total_terms_a.saturating_add(other.stats.total_terms_a);
        self.stats.total_terms_b = self.stats.total_terms_b.saturating_add(other.stats.total_terms_b);
        self.stats.total_terms_c = self.stats.total_terms_c.saturating_add(other.stats.total_terms_c);
        self.stats.max_terms_a = self.stats.max_terms_a.max(other.stats.max_terms_a);
        ...
        fn remap_var(v: VarRef, shift: usize) -> VarRef {
            match v {
                ...
                VarRef::Local(j) => VarRef::Local(j + shift),
            }
        }

        // Offsets into the pooled term arrays.
        let a_off = self.a_terms.len();
        let b_off = self.b_terms.len();
        let c_off = self.c_terms.len();
        let b_shift = b_off.saturating_sub(1);
        ...
        for r in other.constraints {
            ...
            let b = if r.b.start == 0 && r.b.end == 1 {
                ...
                (r.b.start + b_shift)..(r.b.end + b_shift)
            };
            let c = if r.c.start == 0 && r.c.end == 1 {
                0..1
            } else {
                (r.c.start + c_shift)..(r.c.end + c_shift)
            };
