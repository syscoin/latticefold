//! Minimal, apples-to-apples SP1 one-proof benchmark (fold-all-chunks into one Π_fold proof).
//!
//! This example intentionally does **only**:
//! - open/build the SP1 chunk cache
//! - load all chunk matrices into memory
//! - run the hetero-M streaming Π_fold prover under Poseidon-FS
//!
//! Run:
...
fn main() {
    ...
    let cache =
        open_sp1_r1cs_chunk_cache::<R, BabyBear>(&r1cs_path, chunk_size, pad_cols_to_multiple_of)
    ...
    let scheme_main =
        Arc::new(AjtaiCommitmentScheme::<R>::seeded(b"cm_f", MASTER_SEED, 8, ncols));
    ...
    // Public statement binding (placeholder)
    let public_inputs: Vec<<R as PolyRing>::BaseRing> = vec![<R as PolyRing>::BaseRing::from(1u128)];
