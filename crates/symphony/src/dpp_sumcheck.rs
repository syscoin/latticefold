...
impl<F: PrimeField + CanonicalSerialize> Dr1csBuilder<F> {
    ...
    /// File-backed fast path: append a raw A-term (already serialized coefficient bytes).
    #[inline]
    pub fn file_push_a_term_raw(&mut self, coef_bytes: &[u8], idx: u64) -> Result<(), String> {
        let sink = self
            .file_sink
            .as_mut()
            .ok_or_else(|| "file_push_a_term_raw called on non-file-backed builder".to_string())?;
        sink.push_a_term_raw(coef_bytes, idx)?;
        self.file_a_terms = self.file_a_terms.saturating_add(1);
        Ok(())
    }
    ...
    pub fn file_push_b_term_raw(&mut self, coef_bytes: &[u8], idx: u64) -> Result<(), String> {
        let sink = self
            .file_sink
            .as_mut()
            .ok_or_else(|| "file_push_b_term_raw called on non-file-backed builder".to_string())?;
        sink.push_b_term_raw(coef_bytes, idx)?;
        self.file_b_terms = self.file_b_terms.saturating_add(1);
        Ok(())
    }
    ...
    /// File-backed fast path: append a constraint row referencing already-written term ranges.
    #[inline]
    pub fn file_push_constraint_row(
        &mut self,
        a0: u64,
        a1: u64,
        b0: u64,
